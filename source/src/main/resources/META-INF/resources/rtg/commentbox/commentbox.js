function commentboxWidget(ccid, userid, jsonMsg) {
		...
		$(function(){
			...
			if ($(_self.comment_id + ' .comment_editor').first().find('textarea').attr('disabled') != 'disabled') {
				editorClickHandler = function () {
					...
					initEmoticonPopup(_self.comment_id, PF('editor').editor);
					
					var $editorForm = $(_self.comment_id + ' .comment_editor').first();
					var $editor = $editorForm.find('.ql-editor');
					$editor.html('');
					...
					$editorForm.find('.ui-editor-toolbar').fadeIn();
				};
				
				$(_self.comment_id + ' .comment_editor').first().find('.ql-editor').bind('click', editorClickHandler);
			}
		});
	};
	...
	this.showInlineEditor = function (isReshow) {
		...
		_self.resizefunc_edit = function() {
			var offset = $(_self.comment_id + ' .comment-' + comment + ' .body .commentEditorDiv').first().offset();
			$(_self.comment_id + ' .inlineEditorForm').attr('style', '').css(offset).css('position', 'absolute').offset(offset);
		};		
		_self.repositionEditors();
		
		if (!isReshow) {
			$(_self.comment_id + ' .inlineEditorForm .inlineEditor .ql-editor').html($(_self.comment_id + ' .comment-' + comment + ' .body .comment-text').html());
		}
		
		initEmoticonPopup(_self.comment_id, PF('inlineEditorVar').editor);
	...
	this.submitEdit = function () {
		...
		$(_self.comment_id + ' .comment-' + edit_comment_id +' .body .comment-text').html($(_self.comment_id + ' .inlineEditorForm .inlineEditor .ql-editor').html());
		_self.cancelEdit();
	};
	...
	this.showAnswerEditor = function () {
		...
		_self.resizefunc_answer = function() {
			var offset = $(_self.comment_id + ' .treenode-'+ comment +' .ui-treenode-children .commentEditorDivAnswer').last().offset();
			$(_self.comment_id + ' .answerEditorForm').attr('style', '').css(offset).css('position', 'absolute').offset(offset);
		}
		_self.repositionEditors();
		
		initEmoticonPopup(_self.comment_id, PF('answerEditorVar').editor);
	...
	this.initUserTypingEvents = function () {
		$(_self.comment_id + ' .answerEditor .ql-editor').keyup(function() {
